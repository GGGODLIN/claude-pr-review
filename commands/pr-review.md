...
- **重審同一組會覆蓋、但先備份**：群組報告檔名只由目標集合決定、不含 SHA，所以同一組 PR 重審會寫回同一個檔。發布 helper 覆蓋前會把上一輪的 `.audit.md` 與 `.md` 各自改名成 `<原名>.<UTC 時間戳>.bak.md` 留在同一層。身分比對只看 set identity、不看版本清單——新 commit 本來就是重審的理由，比版本會讓合法的新一輪永遠發不出去；換成別組仍會被擋。備份檔的清理由使用者自己決定，流程不自動刪。
